...
interface IInputNumberField extends React.ComponentProps<"input"> {
  label?: string;
  className?: string;
}
...
export const InputNumberField: FC<IInputNumberField> = ({ ...props }) => {
  ...
  return (
    <div className="input_wrapper relative w-full">
      ...
      <input
        className={`${props.className} numberInput w-full py-3 px-5 text-sm border border-gray-300 placeholder-gray-300 rounded-lg focus-visible:outline-none`}
        {...register(props.name!)}
        {...props}
        id={props?.label}
        type="number"
